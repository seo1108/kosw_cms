...
<script>
	...
	/* jQuery(document).ready(function() {
		$('select[name="deptSeq"]').change(function(e) {
			$('input[name="inpDeptSeq"]').val($(this).val());
		});
    }); */
                	...
                	<div class="content">
                		<form name="searchForm" method="get" class="form-horizontal">
							...
							<c:if test="${not empty departmentList}">
								<fieldset>
									<div class="form-group">
										<label class="col-sm-2 control-label">부서 선택</label>
										<div class="col-sm-4">
											<select name="deptSeq" class="selectpicker" data-title="" data-style="btn-default btn-block" data-menu-style="dropdown-blue">
												<option value="">부서 전체</option>
		                    					<c:forEach var="d" items="${departmentList }">
													<option value="${d.deptSeq }" <c:if test="${rank.deptSeq eq d.deptSeq}">selected</c:if> >${d.deptName }</option>
												</c:forEach>
	                    					</select>
										</div>
									</div>
								...
								<table class="table">
									...
									<c:forEach var="r" items="${ranks }">
										<tr>
											<td>${r.ranking }</td>
											<td>
												<a href="statPerUser?uSeq=${r.userSeq }">${r.userName }</a>
											</td>
											<td>${r.nickName }</td>
											<td>${r.custName }</td>
											<td>${r.deptName }</td>
