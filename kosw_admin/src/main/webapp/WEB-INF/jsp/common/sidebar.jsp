...
<c:if test="${fn:indexOf(path, '/buildingList') != -1}">
	<c:set var="title" value="건물 리스트" />
</c:if>

<c:if test="${fn:indexOf(path, '/userList') != -1}">
	<c:set var="title" value="사용자 리스트" />
</c:if>
...
<c:if test="${fn:indexOf(path, '/userAllList') != -1}">
	<c:set var="title" value="회원 리스트" />
</c:if>

<c:if test="${fn:indexOf(path, '/cafeList') != -1}">
	<c:set var="title" value="Cafe 리스트" />
</c:if>
...
<c:if test="${fn:indexOf(path, '/customerOne') != -1}">
	<c:set var="title" value="우리 회사 설정" />
</c:if>
...
<c:if test="${fn:indexOf(path, '/statCafeUser') != -1}">
	<c:set var="title" value="Cafe 개별 통계" />
</c:if>
...
<c:if test="${fn:indexOf(path, '/statCafeCategory') != -1}">
	<c:set var="title" value="Cafe 카테고리 통계" />
</c:if>
...
<c:if test="${fn:indexOf(path, '/beaconAdd') != -1}">
	<c:set var="title" value="비콘 등록/수정" />
</c:if>
...
<c:url var="bbsToAllListURL" value="/bbsToAllList" />
<c:if test="${fn:indexOf(path, '/bbsToAllList') != -1}">
	<c:set var="title" value="전체 공지 게시판" />
</c:if>
...
<c:url var="pushToAllListURL" value="/pushToAllList" />
<c:if test="${fn:indexOf(path, '/pushToAllList') != -1}">
	<c:set var="title" value="전체 푸쉬 메세지" />
</c:if>
...
<div class="sidebar" data-color="orange"
	data-image='<c:url value="/assets/img/full-screen-image-3.jpg" />'>
	<!--
	...
	<div class="sidebar-wrapper">
		...
		<ul class="nav">
			<li><a data-toggle="collapse" href="#myCafeList"> <i
					class="pe-7s-id"></i>
					<p>
						Cafe <b class="caret"></b>
					</p>
			</a>
					...
					<ul class="nav">
						<c:choose>
							<c:when test="${user.superAdmin eq 'true'}">
								<li class="menu"><a href="${cafeListURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 리스트</span>
								</a></li>
							</c:when>
							<c:otherwise>
								<li class="menu"><a href="${cafeOfMineListURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">내 Cafe 리스트</span>
								</a></li>
							</c:otherwise>
						</c:choose>
					</ul>
			...
			<c:if test="${user.superAdmin eq 'true'}">
				<li class="menu"><a href="${userAllListURL }"> <i
						class="pe-7s-users"></i>
						<p class="menu-name">회원 리스트</p>
				</a></li>
			</c:if>

			<li><a data-toggle="collapse" href="#statList"> <i
				...
				<div class="collapse" id="statList">
					<ul class="nav">
						...
						<c:choose>
							<c:when test="${user.superAdmin eq 'true'}">
								<li class="menu"><a href="${statCafeForAdminURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 개별 통계</span>
								</a></li>

								<li class="menu"><a href="${statCafeCategoryForAdminURL }">
										<span class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 카테고리 통계</span>
								</a></li>
							</c:when>
							<c:otherwise>
								<li class="menu"><a href="${statCafeURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 개별 통계</span>
								</a></li>

								<li class="menu"><a href="${statCafeCategoryURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 카테고리 통계</span>
								</a></li>
							</c:otherwise>
			...
			<c:if test="${user.superAdmin eq 'true'}">
				...
				<li><a data-toggle="collapse" href="#characterList"> <i
						class="pe-7s-piggy"></i>
						<p>
							캐릭터 메뉴 <b class="caret"></b>
						</p>
				</a>
					<div class="collapse" id="characterList">
						<ul class="nav">
							<li class="menu"><a href="${characterListURL }"> <span
									class="sidebar-mini">&nbsp;</span> <span
									class="sidebar-normal menu-name">캐릭터 리스트</span>
							</a></li>
							<li class="menu"><a href="${characterAddURL }"> <span
									class="sidebar-mini">&nbsp;</span> <span
									class="sidebar-normal menu-name">캐릭터 등록</span>
							</a></li>
						</ul>
					</div></li>
			...
			-->
				<li class="menu"><a href="${appVersionURL }"> <i
						class="pe-7s-phone"></i>
			...
			<li><a data-toggle="collapse" href="#noticeList"> <i
					class="pe-7s-speaker"></i>
					<p>
						공지 게시판 <b class="caret"></b>
					</p>
			</a>
				<div class="collapse" id="noticeList">
					<ul class="nav">
						<c:choose>
							<c:when test="${user.superAdmin eq 'true'}">
								<li class="menu"><a href="${bbsToAllListURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">전체 공지 게시판</span>
								</a></li>
								<li class="menu"><a href="${cafeNoticeListURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 공지 게시판</span>
								</a></li>
							</c:when>
							<c:otherwise>
								<li class="menu"><a href="${cafeNoticeListURL }"> <span
			...
			<%-- <li class="menu">
			 ...
			 --%>
			<li><a data-toggle="collapse" href="#pushList"> <i
					class="pe-7s-comment"></i>
					<p>
						푸쉬 메세지 <b class="caret"></b>
					</p>
			</a>
				<div class="collapse" id="pushList">
					<ul class="nav">
						<c:choose>
							<c:when test="${user.superAdmin eq 'true'}">
								<li class="menu"><a href="${pushToAllListURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">전체 푸쉬 메세지</span>
								</a></li>
								<li class="menu"><a href="${cafePushListURL }"> <span
										class="sidebar-mini">&nbsp;</span> <span
										class="sidebar-normal menu-name">Cafe 푸쉬 메세지</span>
								</a></li>
							</c:when>
							<c:otherwise>
								<li class="menu"><a href="${cafePushListURL }"> <span
